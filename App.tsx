...
const Sidebar: React.FC<{isOpen: boolean; setIsOpen: (isOpen: boolean) => void;}> = ({isOpen, setIsOpen}) => {
  ...
  const NavLink: React.FC<{item: NavItemWithExact; onClick?: () => void}> = ({ item, onClick }) => { const isActive = item.exact ? location.pathname === item.path : location.pathname.startsWith(item.path) && (location.pathname === item.path || location.pathname.startsWith(item.path + '/')); const IconComponent = item.icon; return ( <Link to={item.path} onClick={onClick} className={`flex items-center px-3 py-3 rounded-md text-sm font-medium transition-colors ${isActive ? 'bg-blue-700 text-white' : 'text-blue-100 hover:bg-blue-600 hover:text-white'}`} > <IconComponent className="h-6 w-6 mr-3 flex-shrink-0" /> {item.name} </Link> ); };
  return ( <> {isOpen && <div className="fixed inset-0 z-30 bg-black/50 lg:hidden" onClick={() => setIsOpen(false)}></div>} <div className={`fixed inset-y-0 left-0 z-40 flex flex-col w-64 bg-blue-800 text-white shadow-lg transform transition-transform duration-300 ease-in-out lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}> <div className="flex items-center justify-between h-20 border-b border-blue-700 px-4"> <img src="https://picsum.photos/seed/bluimportslogo/150/50?grayscale&invert=1" alt="Blu Imports Logo" className="h-10 object-contain"/> {currentUser && <span className="text-xs text-blue-200 truncate" title={currentUser.email || 'Usuário'}>{currentUser.email || 'Usuário'}</span>}</div> <nav className="flex-1 px-4 py-6 space-y-2 overflow-y-auto"> {NAV_ITEMS.map((item) => <NavLink key={item.name} item={item} onClick={() => setIsOpen(false)} />)} </nav> <div className="px-4 py-4 border-t border-blue-700"> <button onClick={async () => { await logout(); setIsOpen(false); }} className="flex items-center w-full px-3 py-3 rounded-md text-sm font-medium text-blue-100 hover:bg-blue-600 hover:text-white transition-colors" > <ArrowLeftOnRectangleIcon className="h-6 w-6 mr-3 flex-shrink-0" /> Sair </button> </div> </div> </> );
};
