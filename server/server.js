...
app.post('/api/clients', authenticateToken, (req, res) => {
    ...
    db.run(sql, params, function(err) {
        ...
        db.get('SELECT * FROM clients WHERE id = $1 AND userId = $2', [clientId, req.user.id], (err, row) => {
            if (err || !row) return res.status(500).json({ message: 'Client saved, but failed to retrieve record.' });
...
app.put('/api/clients/:id', authenticateToken, (req, res) => {
    ...
    db.run(sql, params, function(err) {
        ...
        db.get('SELECT * FROM clients WHERE id = $1 AND userId = $2', [clientId, req.user.id], (err, row) => {
            if (err || !row) return res.status(500).json({ message: 'Client updated, but failed to retrieve record.' });
            res.json({ ...row, isDefaulter: Boolean(row.isDefaulter) });
        });
    });
});
...
app.post('/api/suppliers', authenticateToken, (req, res) => {
    ...
    db.run(sql, params, function(err) {
        ...
        db.get('SELECT * FROM suppliers WHERE id = $1 AND userId = $2', [supplierId, req.user.id], (err, row) => {
            if (err || !row) return res.status(500).json({ message: 'Supplier saved, but failed to retrieve record.' });
...
app.put('/api/suppliers/:id', authenticateToken, (req, res) => {
    ...
    db.run(sql, params, function(err) {
        ...
        db.get('SELECT * FROM suppliers WHERE id = $1 AND userId = $2', [supplierId, req.user.id], (err, row) => {
            if (err || !row) return res.status(500).json({ message: 'Supplier updated, but failed to retrieve record.' });
            res.json(row);
        });
    });
});
